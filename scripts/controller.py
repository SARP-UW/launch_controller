...
class Controller:
    ...
    def __init__(self):
        ...
        self.redlines_armed = False  # Flag to track whether redlines (safety thresholds) are armed
        self.lastPing = time.time()  # Timestamp for the last successful network ping
        self.og_time = 0.0  # Used for time-stamping telemetry
        self.first_time = True  # Flag to indicate if it's the first time telemetry is being sent
        
        # Load appropriate sensor scaling based on control mode ("fill" or "prop")
        with open("/home/pi/controller/" + self._control +"_pt_scale.json") as pt_scalings:
            pt_scaling = json.load(pt_scalings)
            # Initialize appropriate sensor class based on control type (fill or propellant)
        ...
        # Load network addresses for telemetry and command nodes
        addresses= {}
        ...
        # inrtialize command receiver to receive commands from ground control
        ...
        self.soft_arm = False # Flag indiciating whether software arming is active
    ...
    def set_logger(self, name, filename):
        # Set up a logger for writing to a file 
