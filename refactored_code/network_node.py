...
class Network:
    def __init__(self, bind_addr, target_addr):
    ...
    def send(self, encoded_message):
        self.sock.sendto(encoded_message, self.target_addr)

    def receive(self):
        try:
            encoded_received_data, server = self.sock.recvfrom(1024)
        except socket.error:
            return (None, None)
        return (encoded_received_data, server)
