...
class Relays:
    ...
    """ 
    NOTE: CORRECT CONFIG PATH
    """
    # def __init__(self, GPIO, config_path="/home/pi/controller/GSE_master.json"):
    def __init__(self, GPIO, config_path="gse_master.json"):
    ...
    """
    NOTE: IS THIS GPIO PARAMETER NEEDED FOR SOMETHING?
    """
    def arm(self, GPIO):
        """
    ...
    def get_GPIO_map(self):
        ...
        """
        return self.GPIO_MAP[self._control]
    ...
    def set_GPIO(self, GPIO, valve, state):
        if state == 'on':
            GPIO.output(self.GPIO_MAP[self._control][valve], GPIO.HIGH)
        elif state == 'off':
            GPIO.output(self.GPIO_MAP[self._control][valve], GPIO.LOW)
        else:
    ...
    def get_telemetry(self):
        ...
        """
        telemObject = {
            telemetry_config[self._control[0] + 'c_soft_armed']: self.is_armed(),
            telemetry_config[self._control[0] + 'c_state']: states,
            telemetry_config[self._control[0] + 'c_scr_tag']: self._SCR_tag
        }
    ...
    def check_safe_update(self):
        ...
        """
        in_fill_state = self._control == "fill"
