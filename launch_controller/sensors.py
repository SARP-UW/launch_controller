...
class Sensors:
    def __init__(self, pt_scale):
        # setting initial state of this class to be Fill Sensor
        # Read 'control key' from gse_master.json
        with open('gse_master', 'r') as gse_json_f:
            self.gse_json = json.load(gse_json_f)
            self.status = self.gse_json['control_key']
        self.adc = []
